...
class OldModel(CaptionModel):
    ...
    def forward(self, fc_feats, att_feats, seq):
        ...
        for i in range(seq.size(1) - 1):
            ...
            output, state ,att_weight= self.core(xt, fc_feats, att_feats, state) # att_weight for show the attention map
            output = F.log_softmax(self.logit(self.dropout(output)))
            outputs.append(output)

        return torch.cat([_.unsqueeze(1) for _ in outputs], 1),att_weight

    def get_logprobs_state(self, it, tmp_fc_feats, tmp_att_feats, state):
        # 'it' is Variable contraining a word index
        xt = self.embed(it)

        output, state,att_weight = self.core(xt, tmp_fc_feats, tmp_att_feats, state)
    ...
    def sample(self, fc_feats, att_feats, opt={}):
        ...
        for t in range(self.seq_length + 1):
            ...
            output, state ,att_weight= self.core(xt, fc_feats, att_feats, state)
            self.att_list.append(att_weight)
            logprobs = F.log_softmax(self.logit(self.dropout(output)))
        return torch.cat([_.unsqueeze(1) for _ in seq], 1), torch.cat([_.unsqueeze(1) for _ in seqLogprobs], 1),self.att_list
...
class ShowAttendTellCore(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, state):
        ...
        weight = F.softmax(dot) #shape 50 * 1 * 7 * 7, for training
