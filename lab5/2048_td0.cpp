...
private:
	...
	struct lookup {
		...
		void init(int r) {
			...
			int V[4] = { (r >> 0) & 0x0f, (r >> 4) & 0x0f, (r >> 8) & 0x0f, (r >> 12) & 0x0f };//bitshifts the input i bits to the right, then masks by the lower 4 bits.
		...
		static int mvleft(int row[]) {
			...
			for (int i = 0; i < 4; i++) {
				int tile = row[i];
				if (tile == 0) continue;//jump to for loop start point
				row[i] = 0;
				if (tmp != 0) {
					if (tile == tmp) {
						tile = tile + 1;
...
public:
	...
	void update_episode_state(std::vector<state>& path, float alpha = 0.1) const {//TD(0)-state
		update(path.back().before_state(), alpha * -estimate(path.back().before_state())); // update the terminal state
		for (path.pop_back() /* terminal state, already updated */; path.size(); path.pop_back()) {
			state& move = path.back();
			float error = move.value() /* r_t + E[ V(s'_t) ] */ - estimate(move.before_state()); /* V(s_t) */
	...
	void make_statistic(size_t n, const board& b, int score, int unit = 1000) {
		...
		if (n % unit == 0) { // show the training process
			...
			std::fstream out;
			out.open("mean_td0.txt",std::ios::out|std::ios::app);
			if (out.is_open()) {
				out<< mean <<std::endl;
				out.flush();
				out.close();
			}
			out.open("max_td0.txt",std::ios::out|std::ios::app);
			if (out.is_open()) {
				out<< max <<std::endl;
				out.flush();
				out.close();
...
int main(int argc, const char* argv[]) {
	...
	path.reserve(20000);//enlarge capacity
	for (size_t n = 1; n <= total; n++) {
		...
		// update by TD(0)
		tdl.update_episode_state(path, alpha);
