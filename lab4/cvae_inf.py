...
'''
label = 4
model.load_state_dict(torch.load('results/cvae_100.pth'))
...
for i in range(5):
    ...
    sample = Variable(torch.FloatTensor(1, 20).uniform_(-1.0, 1.0)).cuda()

    z = torch.cat([sample, onehot], dim=-1)
    
    sample = model.decode(z)
    save_image(sample.data.view(1, 1, 28, 28),'CVAEoutput_%d.png'%(i),nrow=10)
